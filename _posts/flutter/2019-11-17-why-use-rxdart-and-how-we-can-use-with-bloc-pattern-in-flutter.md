...
Stream은 데이터와 이벤트의 흐름을 나타냅니다. 그렇다면 가장 중요한 것은 무엇입니까? Stream을 사용하면 데이터 및 이벤트 변경 사항을 들을 수 있으며 스트림에서 오는 내용을 Listener와 처리 할 수 ​​있습니다. Flutter에서는 어떻게 적용 할 수 있을까요? 예를 들어, Stream과의 interaction에 대한 최신 Snapshot을 기반으로 자체 빌드하는 StreamBuilder라는 Flutter의 Widget이 있습니다. 그리고, 새로운 데이터 흐름이있는 경우 새 데이터를 처리하기 위해 Widget이 새로 고침됩니다.
...
보시다시피 이 코드는 카운터 증가 및 감소 기능을 제공하지만 BLoC 패턴 또는 스트림을 적용하지는 않았습니다. 이 코드는 간단하게 동작하지만 내부를 살펴보면 UI 코드에 증가 및 감소라는 두 가지 비즈니스 로직이 있음을 알 수 있습니다.
...
만약 이 어플리케이션이 매우 크고 많은 일을 한다고 가정하고 요구 사항이 변경되어 카운터 증가가 두 개 씩 발생한다고 상상해보십시오. 
...
이제 위의 코드를 설명하겠습니다. 먼저 rxdart 라이브러리를 사용하는 CounterBloc이라는 클래스를 만들었습니다. 이 경우, 우리는 카운터가 몇 번부터 시작해야 하는지 알 수 있는 이니셜 카운트를 받아야 합니다. 이 예제에서는 BehaviorSubeject를 선택한 다음 매개 변수가 전달한 데이터로 Subject를 초기화했습니다. 즉, Widget이 Subject의 Listener가되면 스트림을 통해 전달 된 첫 번째 값은 CounterBloc 생성자에 설정된 initialCount가됩니다.
...
- counterObeservable () : Subject의 Observable, 즉 스트림에서 변경이 발생할 때 Widget에 알리는 데 사용되는 객체를 반환합니다.

이제 BLoC 클래스가 만들어졌으니 UI와 통합하는 것을 봅시다.
...
![](https://miro.medium.com/max/1296/1*ZulQfM1GDwB4p6XTR6tn7A.gif)
App running
